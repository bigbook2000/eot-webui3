            ...
            <div class="cell eo_w100">
                ...
                <div class="input">
                    <el-input style="width:100%"
                        v-model="x_device_data['f_config_data']" type="textarea" resize="none"
...
<script setup lang="ts">
    ...
    const updateConfigData = (cpStr: string): string => {
        ...
        for (let s of cpArray) {
            ...
            if (key.startsWith("D")) {
                if (key == "DKey") x_device_data["f_dkey"] = val;
                if (key == "DType") x_device_data["f_dtype"] = val;
                if (key == "DVersion") x_device_data["f_dversion"] = val;
            } else {
                cpStr += key + "=" + val + ";\r\n";
            }
        }
    ...
    const onButtonClick_ConfigSet = async () => {
        ...
        let configData: string = x_device_data["f_config_data"];
        configData = configData.replace(/[\r\n]/g, "");
        configData = updateConfigData(configData);
        x_device_data["f_config_data"] = configData;
